...
export default function ChatAstrologersPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-white border-b border-gray-200 px-4 md:px-10 py-4 md:py-5 flex items-center justify-between overflow-hidden">
        <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 pr-2">
            <h1 className="text-lg xs:text-xl md:text-3xl font-semibold text-gray-900 tracking-wide truncate">
              {t("astrologers_chat.chat_with_astrologers")}
            </h1>
            <div className="mt-1 md:mt-2 h-[2px] md:h-[3px] w-16 md:w-32 bg-yellow-400 rounded-full" />
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 md:space-x-3 shrink-0">
          ...
          <button
            type="button"
            className="flex items-center border border-gray-300 rounded-full px-3 md:px-5 py-1.5 md:py-2.5 text-xs md:text-sm text-gray-800 bg-white hover:bg-gray-50 shadow-sm shrink-0"
            onClick={() => setShowSortModal(true)}
            suppressHydrationWarning>

            <span className="mr-1 md:mr-2 text-gray-600">⚙</span>
            <span className="hidden xs:inline">{t("astrologers_chat.filter")}</span>
          </button>
        </div>
      </div>
      ...
      <div className="md:hidden bg-white border-b border-gray-200 px-4 py-3">
        <form onSubmit={handleSearch} className="flex items-center bg-yellow-400 rounded-full px-4 py-2 shadow-sm">
          ...
          <input
            ...
            suppressHydrationWarning
            className="bg-transparent outline-none text-sm placeholder-gray-700 text-gray-900 flex-1 min-w-0" />
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-6">
        {loading ?
          <div className="flex justify-center items-center py-12">
            ...
            <p className="ml-3 text-sm text-gray-600">{t("astrologers_chat.loading_astrologers")}</p>
          </div> :
          astrologers.length === 0 ?
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-gray-600 text-lg font-medium">{t("astrologers_chat.no_astrologers_found")}</p>
              <p className="text-gray-500 text-sm mt-1">{t("astrologers_chat.try_adjusting_your_filters_or")}</p>
            </div> :
                  ...
                  >
                    {loadingMore ? (
                      <div className="flex items-center">
                        ...
                        {t("astrologers_chat.loading_more")}
                      </div>
                    ) : (
                      t("astrologers_chat.load_more")
                    )}
                  </button>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl flex flex-col overflow-hidden max-h-[80vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-800 tracking-wide uppercase">{t("astrologers_chat.filters")}</h2>
            ...
            <div className="flex flex-1 overflow-hidden">
              <div className="w-1/3 bg-gray-50 border-r border-gray-100 pt-2">
                <button className="w-full text-left relative bg-white py-4 px-5">
                  ...
                  <span className="text-sm font-medium text-gray-900">{t("astrologers_chat.sorting")}</span>
                </button>
              </div>
                ...
                <div className="space-y-4">
                  {SORT_OPTIONS.map((opt) =>
                    <label key={opt.id} className="flex items-center cursor-pointer group">
                      ...
                      <span suppressHydrationWarning className={`text-[15px] ${tempSortBy === opt.id ? 'text-gray-900 font-medium' : 'text-gray-600'}`}>{opt.label}</span>
                    </label>
                  )}
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
              <button type="button" onClick={() => setTempSortBy('popularity')} className="text-teal-700 text-sm font-medium hover:underline">{t("astrologers_chat.reset")}</button>
              <button type="button" onClick={handleApplyFilter} className="bg-yellow-400 hover:bg-yellow-500 text-black font-semibold py-2.5 px-10 rounded-lg text-sm transition-colors shadow-sm">{t("astrologers_chat.apply")}</button>
            </div>
          </div>
        </div>
      }
