...
function ResultsContent() {
  ...
  useEffect(() => {
    fetchNames();
  }, [letter, page, genderFilter]);

  const fetchNames = async () => {
    setLoading(true);
    try {
      const genderParam = genderFilter !== 'all' ? `&gender=${genderFilter}` : '';
      const res = await fetch(`${API_BASE}/baby-names/alphabet/${letter}?page=${page}&limit=50${genderParam}`);
          ...
          <div className="text-left">
            ...
            <div className="text-left">
              <div className="flex items-center justify-start gap-2 text-[#b8962e] text-xs font-bold mb-3 tracking-[0.2em] uppercase">
                ...
                <span className="serif">{t("_letter_.vedic_name_directory")}</span>
                <Crown className="w-3 h-3" />
              </div>
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2 serif">
                {genderFilter !== 'all' ? `${t(`common.${genderFilter.toLowerCase()}`)} ` : ''}{t("_letter_.names_starting_with")}<span className="text-[#b8962e]">&ldquo;{letter}&rdquo;</span>
              </h1>
          ...
          <div className="bg-white/70 backdrop-blur-sm border border-[#d6c89a] rounded-3xl p-6 md:p-8 shadow-lg shadow-[#b8962e]/5">
            <AnimatePresence mode="wait">
              ...
              <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
                  ...
                  <p className="text-gray-400 text-sm mt-1">{t("_letter_.try_a_different_letter_or_filt")}</p>
                </motion.div> :
...
export default function BabyNameResultsPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen py-20 flex flex-col items-center justify-center bg-[#fdf6e3]">
        ...
        <p className="text-[#b8962e] font-medium animate-pulse">Loading Names...</p>
      </div>
    }>
      <ResultsContent />
    </Suspense>
